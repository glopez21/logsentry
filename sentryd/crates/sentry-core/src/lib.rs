...
// serde is the standard Rust serialization framework. Think of it
// as pickle/JSON in Python but compile-time and zero-cost.
// We use it to convert LogEntry ↔ JSON for the SQLite store and
// the Augur REST API.
...
pub struct HubSection {
    pub url: String,                 // Augur hub URL (e.g., "http://augur.local:8080")
    pub api_key: String,             // Optional bearer token
    pub heartbeat_interval: u64,     // Seconds between heartbeat pings
}
...
pub struct StoreSection {
    pub path: String,            // SQLite DB path (e.g., "/var/lib/sentryd/events.db")
    pub retention_days: u32,     // How long to keep sent events
    pub max_pending: u32,        // Max unsent events before dropping oldest
}
